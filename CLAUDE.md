...
- Never use `<Transition mode="out-in">` around a keyed child whose parent re-renders at the same moment (e.g. the week calendar): the leaving element is removed but the new one is never inserted, in real browsers only — jsdom resolves leaves synchronously, so tests can't catch it. Verify week navigation in a real browser after touching it.
